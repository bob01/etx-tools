...
local function resetEsc()
    if Page.values then
        local payload = Page.values
        payload[2] = bit32.bor(payload[2], 0x80)
        protocol.mspWrite(Page.write, payload)
        invalidatePages()
        currentField = 1
    ...
    for i=1,#Page.fields do
        if #Page.values >= Page.minBytes then
            ...
            if f.vals then
                f.value = 0
                for idx=1, #f.vals do
                    local raw_val = Page.values[f.vals[idx] + mspHeaderBytes] or 0
...
local function drawScreen()
    ...
    drawScreenTitle(moduleTitle.." / "..Page.title)
end
...
local function incValue(inc)
    ...
    for idx=1, #f.vals do
        Page.values[f.vals[idx] + mspHeaderBytes] = bit32.rshift(math.floor(f.value*scale + 0.5), (idx-1)*8)
    end
    if f.upd and Page.values then
        f.upd(Page)
    end
    ...
    elseif uiState == uiStatus.pages then
        ...
        elseif pageState == pageStatus.display then
            ...
            elseif event == EVT_VIRTUAL_ENTER then
                if Page then
                    local f = Page.fields[currentField]
                    if Page.values and f.vals and Page.values[f.vals[#f.vals] + mspHeaderBytes] and not f.ro then
